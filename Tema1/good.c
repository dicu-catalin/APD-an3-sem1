...
int cmpfunc_qsort(const void *a, const void *b)
{
	int i;
...
void *thread_function(void *arg)
{
	...
	for (int k = 0; k < generations_count + 1; ++k) {
	    ...
	    qsort(current_generation + start, end - start + 1, sizeof(individual), cmpfunc_qsort);
	...
	if(thread_info.id != 0)
		pthread_exit(NULL);
	/*compute_fitness_function(thread_info.objects, current_generation, object_count, sack_capacity);
